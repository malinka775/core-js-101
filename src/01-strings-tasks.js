...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i += 1) {
    switch (i) {
      case 0: res += `┌${'─'.repeat(width - 2)}┐\n`; break;
      case height - 1: res += `└${'─'.repeat(width - 2)}┘\n`; break;
      default: res += `│${' '.repeat(width - 2)}│\n`;
    }
  }
  return res;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if ((charCode >= 65 && charCode <= 77) || (charCode >= 97 && charCode <= 109)) {
      scryptedCode = str.charCodeAt(i) + 13;
    } else if ((charCode > 77 && charCode <= 90) || (charCode > 109 && charCode <= 122)) {
      scryptedCode = str.charCodeAt(i) - 13;
    } else {
      scryptedCode = charCode;
    }
    res += String.fromCharCode(scryptedCode);
  }
  return res;
}
/**
